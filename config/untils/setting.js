...
class Setting {
  constructor () {
    this.defPath = `${_path}/plugins/impart_cunyx-plugin/def_config/`
    this.defSet = {}

    this.configPath = `${_path}/plugins/impart_cunyx-plugin/config/`
    this.config = {}
  ...
  getFilePath (app, type) {
    if (type === 'defSet') return `${this.defPath}${app}.yaml`
      ...
      try {
        ...
        logger.error(`寸幼萱淫趴缺失默认文件[${app}]${error}`)
      }
  ...
  watch (file, app, type = 'defSet') {
    ...
    watcher.on('change', path => {
      ...
      logger.mark(`[寸幼萱淫趴][修改配置文件][${type}][${app}]`)
      if (this[`change_${app}`]) {
        this[`change_${app}`]()
      }
    })
